...
type Overrides = { copy?: Record<string, string>; treatments?: Record<string, Partial<Treatment>>; pricing?: Record<string, Partial<PriceGroup["items"][number]>>; clinic?: { contactEmail?: string } };
type SiteContent = { copy: Record<string, string>; treatments: Treatment[]; pricing: PriceGroup[]; clinic: { contactEmail: string }; reviews: typeof defaultReviews[number][]; loaded: boolean };
const ContentContext = createContext<SiteContent>({ copy: {}, treatments: defaultTreatments, pricing: defaultPricing, clinic: { contactEmail: "injectoxclinic@gmail.com" }, reviews: [...defaultReviews], loaded: false });
...
function mergeContent(overrides: Overrides) {
  ...
  return { copy: overrides.copy ?? {}, treatments, pricing, clinic: { contactEmail: overrides.clinic?.contactEmail || "injectoxclinic@gmail.com" } };
}
