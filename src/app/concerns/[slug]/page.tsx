...
export default async function ConcernPage({ params }: Props) {
  ...
  return <><PageHero compact eyebrow="Your concern" title={item.title} copy={`${item.short} Compare the recommended routes below.`} index={item.number} /><section id="recommended" className="concern-options shell"><div className="concern-options-head"><div><span className="eyebrow">Recommended treatments</span><h2>Your clearest options.</h2></div><p>Suitability and the final plan are always confirmed in consultation. This guide is a simple starting point, not a diagnosis.</p></div><div className="compare-grid">{options.map((option, i) => option && <article className="compare-card" key={option.slug}><span>Option 0{i+1}</span><h3>{option.name}</h3><p>{option.intro}</p><p><b>Best for:</b> {option.idealFor.slice(0,3).join(", ")}</p><p><b>From:</b> {formatPrice(option.price)} · {option.duration}</p><Button href={`/treatments/${option.slug}`} variant="line">View treatment</Button></article>)}</div><div className="concern-options-cta"><p>Not sure which route is right?</p><Button href={booking.consultation}>Book a consultation</Button></div></section><FinalCTA /></>;
}
